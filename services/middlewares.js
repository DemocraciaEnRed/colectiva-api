...
exports.isPrivateAndHasAccess = async (req, res, next) => {
  try {
    let document = null
    ...
    // if it is the author, continue...
    if (isTheAuthor) {
      next()
      return
    }
    if (!isTheAuthor && document.private && !req.session.user) {
      throw errors.ErrForbidden
    } else if (!isTheAuthor && document.private && req.session.user) {
      // document has a "allow" array of object ids of users that are allowed
      // to see the document and interact. If the user is not in the array
      // then throw an error
      if (!document.allowed || !document.allowed.length) {
        throw errors.ErrForbidden
      } else {
        let allow = false
        document.allowed.forEach((userId) => {
